...
lblMultiAccounts = QtBind.createLabel(gui,'Welcome to ' + pName + '.', 6, 10)

cBoxUseMultiAccount = QtBind.createCheckBox(gui, 'checked_cBoxUseMultiAccount', 'Bypass limit PC', _x, _y)
enableMultiAccount = True
QtBind.setChecked(gui, cBoxUseMultiAccount, True)

_y += 20
cBoxClientMode = QtBind.createCheckBox(gui, 'checked_cBoxClientMode', 'Clientless mode', _x, _y)
...
txtBoxMac = QtBind.createLineEdit(gui, '', _x + 70, _y, 200, 20)

_y += 25
QtBind.createLabel(gui, 'Serial', _x, _y + 3)
txtBoxSerial = QtBind.createLineEdit(gui, '819227034', _x + 70, _y, 200, 20)

_y += 25
QtBind.createLabel(gui, 'PC Name', _x, _y + 3)
txtBoxPCName = QtBind.createLineEdit(gui, 'Administrator', _x + 70, _y, 200, 20)

_y += 25
QtBind.createLabel(gui, 'Profile Name', _x, _y + 3)
txtBoxConfigName = QtBind.createLineEdit(gui, 'Profile 1', _x + 70, _y, 200, 20)

_y += 25
btnSaveConfig = QtBind.createButton(gui,'btnSaveConfig_clicked',"Save config", _x, _y)
...
def parserCommand():
    ...
    if bot_args:
        for i in range(len(bot_args)):
            ...
            if param.startswith('--'):
                if i < len(bot_args) - 1 and '--' not in bot_args[i + 1]:
                    commandParams[bot_args[i].replace('--', '')] = bot_args[i + 1]
                else:
...
def byPassLimitPC():
    packet = bytearray()
    strData1 = 'FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF'
    strData1 += '20 00'

    strData2 = '63 38 30 61 39 34 37 62 37 64 33 31 66 33 66 33 65 66 34 36 39 32 65 34 66 63 37 64 63 36 37 33'.split(' ') # md5
    ...
    strData3 = ' '.join([hex(ord(x)).replace('0x', '') for x in strData3])
    ...
    strData4 = ' '.join([hex(ord(x)).replace('0x', '') for x in strData4])

    strData4 += '0A 10 00'
    
    #strData5 = '32 30 32 31 2D 31 30 2D 30 39 20 31 31 3A 34 31' # time
    strData5 = ' '.join([hex(ord(x)).replace('0x', '') for x in datetime.now().strftime("%m-%d-%Y %H:%M")])

    strData6 = 'FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF FF'
    strData6 += '0A 00'

    #strData7 = '4E 47 55 59 45 4E 54 55 41 4E' # PC name
    #strData7 = 'Administrator'
    strData7 = QtBind.text(gui, txtBoxPCName)
    strData7 = ' '.join([hex(ord(x)).replace('0x', '') for x in strData7])

    strData = strData1 + strData2 + strData3 + strData4 + strData5 + strData6 + strData7
    strData = strData.replace(' ','')
    strDataLen = len(strData)
